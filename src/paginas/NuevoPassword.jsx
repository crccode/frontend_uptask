...
const NuevoPassword = () => {
  // DEFINIMOS STATE Y ALERTAS
  const [password, setPassword] = useState('')
  // TOKEN TRUE MUESTRA EL FORMULARIO
  const [tokenValido, setTokenValido] = useState(false)
  const [alerta, setAlerta] = useState({})
  // PASSWORD MODEIFICADO TRUE MUESTRA EL LINK INICIAR SECION
  const [passwordModificado, setPasswordModificado] = useState(false)

  const params = useParams()
  const { token } = params

  // VERIFICAMOS QUE EL TOKEN SEA IGUAL AL QUE LE ESTAMOS ENVIANDO
  useEffect(() => {
      const comprobarToken = async () => {
          try {
            // TODO: MOVER CLIENT AXIOS
  ...
  // BOTON DEL FORMULARIO RESETEAR PASSWORD
